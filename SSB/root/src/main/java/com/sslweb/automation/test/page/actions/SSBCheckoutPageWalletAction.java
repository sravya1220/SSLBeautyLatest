...
public class SSBCheckoutPageWalletAction extends GlobalExceptionHandler {
	...
	public void EnteringWalletDetails(String testCaseName) {
		try {
			WebElementOperationsWeb.waitForPageLoad(driver, 60);
			WebElementOperationsWeb.park(15);
			...
			WebElementOperationsWeb.captureScreenShotOnPass(driver, testCaseName, "EnteringWalletDetails");
	...
	public void removeProduct(String testCaseName) {
		try {
			ssbcheckoutwalletpay.cartClick();
			WebElementOperationsWeb.park(3);
			ssbcheckoutwalletpay.productRemove();
			ssbcheckoutwalletpay.productRemoveBtn();
