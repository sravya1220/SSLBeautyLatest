...
public class SSBCheckoutPageCreditCardPayTest extends AbstractTest {
	...
	public void verifyCreditcardpayatcheckout() {
		...
		//	getSslDecryptUrl();
			...
			ssbcheckoutpagecreditcardpay.NavigatetoCartPage(TEST_CASE_NAME);
			ssbcheckoutpagecreditcardpay.ClickingonProceedNow(TEST_CASE_NAME);
			ssbcheckoutpagecreditcardpay.AddingAddress(REGISTRATION_SHEET, TEST_CASE_NAME);
			ssbcheckoutpagecreditcardpay.EnteringCreditCardDetails(CHECKOUT_PAGE, TEST_CASE_NAME, SERIAL_NO);
			ssbcheckoutpagecreditcardpay.removeProduct(TEST_CASE_NAME);
